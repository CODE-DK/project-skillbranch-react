...
refreshData();
timer.innerHTML = `${timeView(data.min)}:${timeView(data.sec)}`;
...
function check(val, def) {
    if (!(val).match(regEx) || +val < 0 || +val > 60) {
        alert('Only numbers are available! Value should be 0-60 only!');
        val.value = def;
    }
}
...
function refreshData() {
    data.min = +min.value;
    data.sec = +sec.value;
    data.shortT = +shortT.value;
    data.longT = +longT.value;
}
